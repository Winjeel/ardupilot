...
void AC_PolyFence_loader::init()
{
    ...
    _old_total = _total;
    _perf_BreachCalc = hal.util->perf_alloc(AP_HAL::Util::PC_ELAPSED, "PolyFence_Breach");
}
...
bool AC_PolyFence_loader::breached() const
{
    ...
    #if POLYFENCE_BREACH_TIMINGS
    void *istate = hal.scheduler->disable_interrupts_save();
    #endif
    hal.util->perf_begin(_perf_BreachCalc);
    uint64_t start_time = AP_HAL::micros64();
    // check if vehicle is outside the polygon fence
    // this is done with the lat/lon coordinates of the vehicle
    struct Location loc;
    if (AP::ahrs().get_position(loc)) {
        have_position = true;
        is_breached = breached(loc);
    }

    uint64_t end_time = AP_HAL::micros64();
    hal.util->perf_end(_perf_BreachCalc);

    AP::logger().Write("PLYF", 
                        "TimeUS,Diff,Brchd,Pos",
                        "----",
                        "----",
                        "QQBb",
                        AP_HAL::micros64(),
...
bool AC_PolyFence_loader::scale_latlon_from_origin(const Location &origin, Vector2l point, Vector2f &pos_cm)
{
    Location tmp_loc;
    tmp_loc.lat = point.x;
    tmp_loc.lng = point.y;
