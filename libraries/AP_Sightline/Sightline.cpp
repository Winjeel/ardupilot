...
void Sightline::init(void) {
    ...
    nextDoSnapshotTime = driver_init_time;
    nextSetMetadataTime = driver_init_time;
    nextGetVersionTime = driver_init_time;
}
...
void Sightline::update(void) {
    ...
    if (SIGHTLINE_DEBUG) {
        tick = ((tick + 1) % 10);
        if (tick == 0) {
            gcs().send_text(MAV_SEVERITY_WARNING, "[%lums] SL: using uart %d\n", timeNow, serial_manager.get_index(uart));
        }
    }
    ...
    if (!uart->is_initialized()) {
        ...
        gcs().send_text(MAV_SEVERITY_WARNING, "    SL: uart !init");
        return;
    }
    ...
    while (msgType != SL_MsgId_None) {
        ...
        case SL_MsgId_VersionNumber: {
            // Sightline boards send the VersionNumber message over serial when
            // Command and Control interface is initialised.
            isConnected = true;
            ...
            gcs().send_text(MAV_SEVERITY_DEBUG, "    SL:     hwType=%d", versionMsg.hwType);
        ...
        size_t eaten = msgBuffer.consumeMsg();
        gcs().send_text(MAV_SEVERITY_DEBUG, "    SL:     consumed=%lu:", eaten);

        msgType = msgBuffer.assess();
    }


    // TODO: send GetVersion while we wait?
    if (!isConnected) {
        const uint32_t kGetVersionPeriod = 5000; // millis (0 to disable)
        if (kGetVersionPeriod && (timeNow > nextGetVersionTime)) {
        ...
        const uint32_t kDoSnapshotPeriod = 0; //static_cast<uint32_t>(frequency.get() * 1000); // Hz to millis
        const uint32_t kSetMetadataPeriod = 0; // millis (0 to disable)
        const uint32_t kGetVersionPeriod = 10000; // millis (0 to disable)
        ...
        if (kSetMetadataPeriod && (timeNow > nextSetMetadataTime)) {
            // TODO: Populate fully...
        ...
        if (kDoSnapshotPeriod && (timeNow > nextDoSnapshotTime)) {
