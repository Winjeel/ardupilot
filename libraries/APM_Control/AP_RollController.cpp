...
const AP_Param::GroupInfo AP_RollController::var_info[] = {
    ...
    // @Increment: 10.0
    // @User: Advanced
    AP_GROUPINFO("SRMAX", 7, AP_RollController, _slew_rate_max, 300.0f),
    ...
    // @Increment: 0.1
    // @User: Advanced
    AP_GROUPINFO("SRTAU", 8, AP_RollController, _slew_rate_tau, 0.5f),
...
int32_t AP_RollController::_get_rate_out(float desired_rate, float scaler, bool disable_integrator)
{
	...
	if (!disable_integrator && ki_rate > 0) {
		// only integrate if gain and time step are positive and airspeed above the minimum where we could be flying.
