...
void QuadPlane::control_loiter()
{
    ...
    if (plane.control_mode == &plane.mode_qland) {
        ...
        float height_above_ground = plane.relative_ground_altitude(plane.g.rangefinder_landing == land_hagl_source::RANGEFINDER);
...
void QuadPlane::update_throttle_suppression(void)
{
    ...
    if (plane.relative_ground_altitude(plane.g.rangefinder_landing == land_hagl_source::RANGEFINDER) > 5) {
        return;
    }
